...
public class UsuarioBeanDAO implements DAO<Object, Usuario> {
	...
	public List<Usuario> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Usuario> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
